...
public class XTSEncryption
{
    ...
    public void EncryptXTS()
    {
        ...
        var tweakedPlaintext = XORMatrixOperation(original_plaintext, tweak);
        PrintMatrix(tweakedPlaintext, "Plaintext XOR with tweak");

        // Encrypt the tweaked plaintext using AES rounds
        var resultMatrix = XORMatrixOperation(tweakedPlaintext, key);
        PrintMatrix(resultMatrix, "Initial XOR between tweaked plaintext and initial key matrix");

        // Proceed with the rest of the AES encryption rounds
        ...
        PrintMatrix(roundKey, "Encrypted result matrix before applying the tweak again");

        // XOR ciphertext with the tweak
        var tweakedCiphertext = XORMatrixOperation(roundKey, tweak);
        PrintMatrix(tweakedCiphertext, "Final ciphertext after applying the tweak");
    }

    public void DecryptXTS()
    {
        // Generate tweak based on block number (for demonstration, we're using a single block)
        var tweak = GenerateTweak(1);

        // XOR ciphertext with the tweak
        var tweakedCiphertext = XORMatrixOperation(ciphertext, tweak);
        PrintMatrix(tweakedCiphertext, "Ciphertext XOR with tweak");

        // Start decryption rounds
        var roundKey = XORMatrixOperation(tweakedCiphertext, key_3);
        ...
        var decryptedMatrix = XORMatrixOperation(thirdRound, key_0);

        // XOR plaintext with the tweak
        var finalPlaintext = XORMatrixOperation(decryptedMatrix, tweak);
        PrintMatrix(finalPlaintext, "Decrypted plaintext after applying the tweak");
    }
